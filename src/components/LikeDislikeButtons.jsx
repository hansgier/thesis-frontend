...
export const LikeDislikeButtons = React.memo(({ reactions }) => {
    ...
    return (
        <>
            ...
            <div className="flex gap-1 h-full items-center justify-center">
                ...
                <span
                    className={ `flex h-auto items-center select-none ${ reaction === "like" ? "text-[#0284c7]" : "text-[#454545]" } text-xs md:text-sm` }>{ reactions.likes }</span>
            </div>
            {/*----------Dislike Button----------*/ }
            <div className="flex gap-1 h-full items-center justify-center">
                <BiDislike size={ 17 } onClick={ handleDislike }
                           ...
                           className="hover:cursor-pointer hover:scale-125 hover:duration-200 hover:transition-all" />
                <span
                    className={ `flex h-full items-center justify-center select-none ${ reaction === "dislike" ? "text-[#db2777]" : "text-[#454545]" } text-xs md:text-sm` }>{ reactions.dislikes }</span>
            </div>
        </>
    );
});